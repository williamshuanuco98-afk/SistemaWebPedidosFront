...
export function saveStorageConfig() {
  ...
  if (!newPath || !newGuiasPath) {
    alert('Por favor ingrese rutas de carpeta válidas para pedidos y guías.');
    return;
  }
  ...
  if (feedback) {
    ...
    feedback.innerHTML = `<i class="bi bi-check-circle-fill me-2 fs-5"></i><div><strong>¡Rutas guardadas exitosamente!</strong><br>Pedidos: <code>${newPath}</code><br>Guías: <code>${newGuiasPath}</code></div>`;
